...
Abbreviation N_rect := N_rect (only parsing).
Abbreviation N_rec := N_rec (only parsing).
Abbreviation N_ind := N_ind (only parsing).
Abbreviation N0 := N0 (only parsing).
Abbreviation Npos := N.pos (only parsing).

Abbreviation Ndouble_plus_one := N.succ_double (only parsing).
Abbreviation Nplus := N.add (only parsing).
Abbreviation Nminus := N.sub (only parsing).
Abbreviation Nmult := N.mul (only parsing).

Abbreviation nat_of_N := N.to_nat (only parsing).
Abbreviation N_of_nat := N.of_nat (only parsing).
Abbreviation Nrect := N.peano_rect (only parsing).
Abbreviation Nrect_base := N.peano_rect_base (only parsing).
Abbreviation Nrect_step := N.peano_rect_succ (only parsing).
Abbreviation Nind := N.peano_ind (only parsing).
Abbreviation Nrec := N.peano_rec (only parsing).
Abbreviation Nrec_base := N.peano_rec_base (only parsing).
Abbreviation Nrec_succ := N.peano_rec_succ (only parsing).

Abbreviation Npred_minus := N.pred_sub (only parsing).
Abbreviation Ppred_N_spec := N.pos_pred_spec (only parsing).
Abbreviation Ppred_Nsucc := N.pos_pred_succ (only parsing).
Abbreviation Nplus_0_l := N.add_0_l (only parsing).
Abbreviation Nplus_0_r := N.add_0_r (only parsing).
Abbreviation Nplus_comm := N.add_comm (only parsing).
Abbreviation Nplus_assoc := N.add_assoc (only parsing).
Abbreviation Nplus_succ := N.add_succ_l (only parsing).
Abbreviation Nsucc_0 := N.succ_0_discr (only parsing).
Abbreviation Nminus_N0_Nle := N.sub_0_le (only parsing).
Abbreviation Nminus_0_r := N.sub_0_r (only parsing).
Abbreviation Nminus_succ_r:= N.sub_succ_r (only parsing).
Abbreviation Nmult_0_l := N.mul_0_l (only parsing).
Abbreviation Nmult_1_l := N.mul_1_l (only parsing).
Abbreviation Nmult_1_r := N.mul_1_r (only parsing).
Abbreviation Nmult_comm := N.mul_comm (only parsing).
Abbreviation Nmult_assoc := N.mul_assoc (only parsing).
Abbreviation Nmult_plus_distr_r := N.mul_add_distr_r (only parsing).
Abbreviation Nle_0 := N.le_0_l (only parsing).
Abbreviation Ncompare_Eq_eq := N.compare_eq (only parsing).
Abbreviation Ncompare_eq_correct := N.compare_eq_iff (only parsing).
Abbreviation Nle_lteq := N.lt_eq_cases (only parsing).
Abbreviation Ncompare_0 := N.compare_0_r (only parsing).
Abbreviation Ndouble_div2 := N.div2_double (only parsing).
Abbreviation Ndouble_plus_one_div2 := N.div2_succ_double (only parsing).
Abbreviation Ndouble_plus_one_inj := N.succ_double_inj (only parsing).
Abbreviation Nlt_not_eq := N.lt_neq (only parsing).
Abbreviation Ngt_Nlt := N.gt_lt (only parsing).
...
Proof.
  exact (proj1 (N.add_cancel_l m p n)).
Qed.
Lemma Nmult_Sn_m n m : N.succ n * m = m + n * m.
Proof.
  exact (eq_trans (N.mul_succ_l n m) (N.add_comm _ _)).
Qed.
Lemma Nmult_plus_distr_l n m p : p * (n + m) = p * n + p * m.
Proof.
  exact (N.mul_add_distr_l p n m).
Qed.
Lemma Nmult_reg_r n m p : p <> 0 -> n * p = m * p -> n = m.
Proof.
  exact (fun H => proj1 (N.mul_cancel_r n m p H)).
Qed.
Lemma Ncompare_antisym n m : CompOpp (n ?= m) = (m ?= n).
Proof.
  exact (eq_sym (N.compare_antisym n m)).
Qed.
