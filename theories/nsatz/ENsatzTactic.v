...
Section ensatz.
  ...
  *)
  ...
  Local Fixpoint mk_HCond
    {A : Type} (Interp : A -> R) (l : list (bool * A)) {struct l} : Prop :=
    match l with
    | nil => True
    | (true, a) :: l0 => (mk_HCond Interp l0)
    | (false, a) :: l0 => ((Interp a) == 0) /\ (mk_HCond Interp l0)
    end.
  ...
  (*
   ...
   lq = [qn+m; ... q1]
   -------------------------------------------------------
   "mk_HCond" build the conjonction of "pi = 0" forall pi where "bi = false"
   -------------------------------------------------------
...
Module Tag_pl.
  ...
  Local Definition tag_polynome l n :=
    let l := List.rev l in
    let hdn_true := set_true (firstn n l) in
    let tln_false := set_false (skipn n l) in
  ...
  Ltac ensatz_solve cr R p2 lp2 n fv info :=
    ...
    NsatzTactic.nsatz_call 1%N info 0%Z p2 lp2
      ltac:(fun c r lq lci =>
              ...
              assert (Hg2: mk_HCond (PEevalR lv) pl);
...
Module Eensatz_solve.
  ...
  (* Instantiate existential variables with the cofactors  *)
  ...
  Module Reifi.
    ...
    Local Ltac areify R Tring lvar term:=
      ...
      | (Ring (T:=?R) (ring0:=?ring0) (ring1:=?ring1)
          ...
          let _ := match goal with _ => convert opp op end in
          let E' := areify R Tring lvar E in
          constr:(opp_mpoly E')
      end.
