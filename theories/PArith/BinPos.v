...
Proof.
  ...
  - apply eq_dep_eq_positive.
    cut (1=1).
    + pattern 1 at 1 2 5, q'. destruct q' as [|p ?].
      * trivial.
      * destruct p; intros; discriminate.
    + trivial.
  - apply eq_dep_eq_positive.
    cut (succ p=succ p).
    + pattern (succ p) at 1 2 5, q'. destruct q' as [|? q'].
...
Abbreviation mul_eq_1 := mul_eq_1_l.
...
Proof.
  destruct n.
  - trivial.
  - simpl pred. rewrite pred_succ. apply of_nat_succ.
Qed.
...
Abbreviation positive := positive (only parsing).
Abbreviation positive_rect := positive_rect (only parsing).
Abbreviation positive_rec := positive_rec (only parsing).
Abbreviation positive_ind := positive_ind (only parsing).
Abbreviation xI := xI (only parsing).
Abbreviation xO := xO (only parsing).
Abbreviation xH := xH (only parsing).

Abbreviation IsNul := Pos.IsNul (only parsing).
Abbreviation IsPos := Pos.IsPos (only parsing).
Abbreviation IsNeg := Pos.IsNeg (only parsing).

Abbreviation Pplus := Pos.add (only parsing).
Abbreviation Pplus_carry := Pos.add_carry (only parsing).
Abbreviation Pmult_nat := (Pos.iter_op plus) (only parsing).
Abbreviation nat_of_P := Pos.to_nat (only parsing).
Abbreviation P_of_succ_nat := Pos.of_succ_nat (only parsing).
Abbreviation Pdouble_minus_one := Pos.pred_double (only parsing).
Abbreviation positive_mask := Pos.mask (only parsing).
Abbreviation positive_mask_rect := Pos.mask_rect (only parsing).
Abbreviation positive_mask_ind := Pos.mask_ind (only parsing).
Abbreviation positive_mask_rec := Pos.mask_rec (only parsing).
Abbreviation Pdouble_plus_one_mask := Pos.succ_double_mask (only parsing).
Abbreviation Pdouble_minus_two := Pos.double_pred_mask (only parsing).
Abbreviation Pminus_mask := Pos.sub_mask (only parsing).
Abbreviation Pminus_mask_carry := Pos.sub_mask_carry (only parsing).
Abbreviation Pminus := Pos.sub (only parsing).
Abbreviation Pmult := Pos.mul (only parsing).
Abbreviation iter_pos := @Pos.iter (only parsing).
Abbreviation Psize := Pos.size_nat (only parsing).
Abbreviation Psize_pos := Pos.size (only parsing).
Abbreviation Pcompare x y m := (Pos.compare_cont m x y) (only parsing).
Abbreviation positive_eq_dec := Pos.eq_dec (only parsing).
Abbreviation xI_succ_xO := Pos.xI_succ_xO (only parsing).
Abbreviation Psucc_o_double_minus_one_eq_xO :=
 Pos.succ_pred_double (only parsing).
Abbreviation Pdouble_minus_one_o_succ_eq_xI :=
 Pos.pred_double_succ (only parsing).
Abbreviation xO_succ_permute := Pos.double_succ (only parsing).
Abbreviation double_moins_un_xO_discr :=
 Pos.pred_double_xO_discr (only parsing).
Abbreviation Psucc_not_one := Pos.succ_not_1 (only parsing).
Abbreviation Psucc_pred := Pos.succ_pred_or (only parsing).
Abbreviation Pplus_carry_spec := Pos.add_carry_spec (only parsing).
Abbreviation Pplus_comm := Pos.add_comm (only parsing).
Abbreviation Pplus_succ_permute_r := Pos.add_succ_r (only parsing).
Abbreviation Pplus_succ_permute_l := Pos.add_succ_l (only parsing).
Abbreviation Pplus_no_neutral := Pos.add_no_neutral (only parsing).
Abbreviation Pplus_carry_plus := Pos.add_carry_add (only parsing).
Abbreviation Pplus_reg_r := Pos.add_reg_r (only parsing).
Abbreviation Pplus_reg_l := Pos.add_reg_l (only parsing).
Abbreviation Pplus_carry_reg_r := Pos.add_carry_reg_r (only parsing).
Abbreviation Pplus_carry_reg_l := Pos.add_carry_reg_l (only parsing).
Abbreviation Pplus_assoc := Pos.add_assoc (only parsing).
Abbreviation Pplus_xO := Pos.add_xO (only parsing).
Abbreviation Pplus_xI_double_minus_one := Pos.add_xI_pred_double (only parsing).
Abbreviation Pplus_xO_double_minus_one := Pos.add_xO_pred_double (only parsing).
Abbreviation Pplus_diag := Pos.add_diag (only parsing).
Abbreviation PeanoView := Pos.PeanoView (only parsing).
Abbreviation PeanoOne := Pos.PeanoOne (only parsing).
Abbreviation PeanoSucc := Pos.PeanoSucc (only parsing).
Abbreviation PeanoView_rect := Pos.PeanoView_rect (only parsing).
Abbreviation PeanoView_ind := Pos.PeanoView_ind (only parsing).
Abbreviation PeanoView_rec := Pos.PeanoView_rec (only parsing).
Abbreviation peanoView_xO := Pos.peanoView_xO (only parsing).
Abbreviation peanoView_xI := Pos.peanoView_xI (only parsing).
Abbreviation peanoView := Pos.peanoView (only parsing).
Abbreviation PeanoView_iter := Pos.PeanoView_iter (only parsing).
Abbreviation eq_dep_eq_positive := Pos.eq_dep_eq_positive (only parsing).
Abbreviation PeanoViewUnique := Pos.PeanoViewUnique (only parsing).
Abbreviation Prect := Pos.peano_rect (only parsing).
Abbreviation Prect_succ := Pos.peano_rect_succ (only parsing).
Abbreviation Prect_base := Pos.peano_rect_base (only parsing).
Abbreviation Prec := Pos.peano_rec (only parsing).
Abbreviation Pind := Pos.peano_ind (only parsing).
Abbreviation Pcase := Pos.peano_case (only parsing).
Abbreviation Pmult_1_r := Pos.mul_1_r (only parsing).
Abbreviation Pmult_Sn_m := Pos.mul_succ_l (only parsing).
Abbreviation Pmult_xO_permute_r := Pos.mul_xO_r (only parsing).
Abbreviation Pmult_xI_permute_r := Pos.mul_xI_r (only parsing).
Abbreviation Pmult_comm := Pos.mul_comm (only parsing).
Abbreviation Pmult_plus_distr_l := Pos.mul_add_distr_l (only parsing).
Abbreviation Pmult_plus_distr_r := Pos.mul_add_distr_r (only parsing).
Abbreviation Pmult_assoc := Pos.mul_assoc (only parsing).
Abbreviation Pmult_xI_mult_xO_discr := Pos.mul_xI_mul_xO_discr (only parsing).
Abbreviation Pmult_xO_discr := Pos.mul_xO_discr (only parsing).
Abbreviation Pmult_reg_r := Pos.mul_reg_r (only parsing).
Abbreviation Pmult_reg_l := Pos.mul_reg_l (only parsing).
Abbreviation Pmult_1_inversion_l := Pos.mul_eq_1_l (only parsing).
Abbreviation iter_pos_swap_gen := Pos.iter_swap_gen (only parsing).
Abbreviation iter_pos_swap := Pos.iter_swap (only parsing).
Abbreviation iter_pos_succ := Pos.iter_succ (only parsing).
Abbreviation iter_pos_plus := Pos.iter_add (only parsing).
Abbreviation iter_pos_invariant := Pos.iter_invariant (only parsing).
Abbreviation Pcompare_refl_id := Pos.compare_cont_refl (only parsing).
Abbreviation Pcompare_eq_iff := Pos.compare_eq_iff (only parsing).
Abbreviation Pcompare_Gt_Lt := Pos.compare_cont_Gt_Lt (only parsing).
Abbreviation Pcompare_eq_Lt := Pos.compare_lt_iff (only parsing).
Abbreviation Pcompare_Lt_Gt := Pos.compare_cont_Lt_Gt (only parsing).

Abbreviation Pcompare_antisym := Pos.compare_cont_antisym (only parsing).
Abbreviation ZC1 := Pos.gt_lt (only parsing).
Abbreviation ZC2 := Pos.lt_gt (only parsing).
Abbreviation Pcompare_p_Sp := Pos.lt_succ_diag_r (only parsing).
Abbreviation Pcompare_1 := Pos.nlt_1_r (only parsing).
Abbreviation Plt_1 := Pos.nlt_1_r (only parsing).
Abbreviation Pplus_compare_mono_l := Pos.add_compare_mono_l (only parsing).
Abbreviation Pplus_compare_mono_r := Pos.add_compare_mono_r (only parsing).
Abbreviation Pplus_lt_mono_l := Pos.add_lt_mono_l (only parsing).
Abbreviation Pplus_lt_mono_r := Pos.add_lt_mono_r (only parsing).
Abbreviation Pplus_lt_mono := Pos.add_lt_mono (only parsing).
Abbreviation Pplus_le_mono_l := Pos.add_le_mono_l (only parsing).
Abbreviation Pplus_le_mono_r := Pos.add_le_mono_r (only parsing).
Abbreviation Pplus_le_mono := Pos.add_le_mono (only parsing).
Abbreviation Pmult_compare_mono_l := Pos.mul_compare_mono_l (only parsing).
Abbreviation Pmult_compare_mono_r := Pos.mul_compare_mono_r (only parsing).
Abbreviation Pmult_lt_mono_l := Pos.mul_lt_mono_l (only parsing).
Abbreviation Pmult_lt_mono_r := Pos.mul_lt_mono_r (only parsing).
Abbreviation Pmult_lt_mono := Pos.mul_lt_mono (only parsing).
Abbreviation Pmult_le_mono_l := Pos.mul_le_mono_l (only parsing).
Abbreviation Pmult_le_mono_r := Pos.mul_le_mono_r (only parsing).
Abbreviation Pmult_le_mono := Pos.mul_le_mono (only parsing).
Abbreviation Plt_plus_r := Pos.lt_add_r (only parsing).
Abbreviation Plt_not_plus_l := Pos.lt_not_add_l (only parsing).
Abbreviation Pminus_mask_succ_r := Pos.sub_mask_succ_r (only parsing).
Abbreviation Pminus_mask_carry_spec := Pos.sub_mask_carry_spec (only parsing).
Abbreviation Pminus_succ_r := Pos.sub_succ_r (only parsing).
Abbreviation Pminus_mask_diag := Pos.sub_mask_diag (only parsing).

Abbreviation Pplus_minus_eq := Pos.add_sub (only parsing).
Abbreviation Pmult_minus_distr_l := Pos.mul_sub_distr_l (only parsing).
Abbreviation Pminus_lt_mono_l := Pos.sub_lt_mono_l (only parsing).
Abbreviation Pminus_compare_mono_l := Pos.sub_compare_mono_l (only parsing).
Abbreviation Pminus_compare_mono_r := Pos.sub_compare_mono_r (only parsing).
Abbreviation Pminus_lt_mono_r := Pos.sub_lt_mono_r (only parsing).
Abbreviation Pminus_decr := Pos.sub_decr (only parsing).
Abbreviation Pminus_xI_xI := Pos.sub_xI_xI (only parsing).
Abbreviation Pplus_minus_assoc := Pos.add_sub_assoc (only parsing).
Abbreviation Pminus_plus_distr := Pos.sub_add_distr (only parsing).
Abbreviation Pminus_minus_distr := Pos.sub_sub_distr (only parsing).
Abbreviation Pminus_mask_Lt := Pos.sub_mask_neg (only parsing).
Abbreviation Pminus_Lt := Pos.sub_lt (only parsing).
Abbreviation Pminus_Eq := Pos.sub_diag (only parsing).
Abbreviation Psize_monotone := Pos.size_nat_monotone (only parsing).
Abbreviation Psize_pos_gt := Pos.size_gt (only parsing).
Abbreviation Psize_pos_le := Pos.size_le (only parsing).
