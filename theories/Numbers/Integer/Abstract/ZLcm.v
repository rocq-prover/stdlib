...
From Stdlib Require Import ZAxioms ZMulOrder ZSgnAbs ZGcd ZDivTrunc ZDivFloor.
...
*)
  ...
  Proof.
    ...
    apply divide_antisym_nonneg; trivial.
    - apply lcm_nonneg.
    - now apply lcm_least.
    - apply H.
      + apply divide_lcm_l.
  ...
  Proof.
    intros. apply lcm_unique; trivial.
    - order.
    - apply divide_refl.
    - apply divide_0_r.
  Qed.
  ...
  Proof.
    ...
    intros q. split.
    - split; trivial. now transitivity m.
    - now destruct 1.
  Qed.
  ...
  Proof.
    intros n m p.
    destruct (eq_decidable p 0) as [Hp|Hp];[|destruct (eq_decidable (gcd n m) 0) as [Hg|Hg]].
    ...
    - unfold lcm.
      ...
      + rewrite divide_div_mul_exact; trivial.
        * rewrite abs_mul.
